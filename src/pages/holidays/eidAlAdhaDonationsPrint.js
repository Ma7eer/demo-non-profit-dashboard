    ...
    <div id="div1">
      ...
      <table style={{ backgroundColor: 'white' }} className="myTable">
        <tbody>
          <tr>
            <td>
              <h1 className="printHeader">بيانات اسر عيد الاضحى</h1>
            </td>
          </tr>
        </tbody>
      </table>
          ...
          {loading ? (
            ...
            data.map((d, index) => (
              <tr key={index}>
                ...
                <td>{d.eidAlAdha[0].date}</td>
                <td>
                  <img
                    src={d.eidAlAdha[0].eidAlAdhaSignature}
                    alt="signature"
                    // width="100px"
                    height="25px"
                  />
...
export default class EidAlAdhaDonationsPrintTable extends React.Component {
  state = {
    formId: '',
    data: [],
    loading: true
  ...
  componentDidMount() {
    let u = url + '/family/eidAlAdhaPrint'
    axios
      .get(u, {
